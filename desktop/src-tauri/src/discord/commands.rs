// ─── Десктоп: полная реализация через Discord IPC ────────────────────────────
    ...
    pub fn discord_set_activity(
        ...
        activity = match mode {
            DiscordRpcMode::Track => activity
                .details(&track.title)
                .state(if is_playing { track.artist.as_str() } else { "Paused" }),
            DiscordRpcMode::Artist => {
                let a = activity.details(&track.artist);
                if is_playing { a } else { a.state("Paused") }
            }
            DiscordRpcMode::Activity => {
                if is_playing { activity } else { activity.details("Paused") }
            }
        };
...
// ─── Android: Discord IPC недоступен, возвращаем стаб-ошибки ─────────────────
#[cfg(target_os = "android")]
mod android_stubs {
    use std::sync::Mutex;

    /// Пустой стейт — на Android Discord нет
    pub struct DiscordState {
        pub client: Mutex<()>,
    }
    ...
    #[tauri::command]
    pub fn discord_connect() -> Result<bool, String> {
        Err("Discord IPC is not available on Android".into())
    }
    ...
    #[tauri::command]
    pub fn discord_set_activity(_track: DiscordTrackInfo) -> Result<(), String> {
        Err("Discord IPC is not available on Android".into())
    }
