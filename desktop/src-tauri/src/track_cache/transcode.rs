...
/// На Android ffmpeg недоступен — acquire сразу возвращает None.
#[cfg(target_os = "android")]
pub async fn acquire_ffmpeg(_install_dir: &std::path::Path) -> Option<std::path::PathBuf> {
    None
}
...
///
/// На Android spawn дочерних процессов запрещён — функция всегда возвращает None.
#[cfg(not(target_os = "android"))]
...
#[cfg(target_os = "android")]
async fn download_ffmpeg(_install_dir: &Path) -> Option<PathBuf> {
    // На Android нельзя запускать дочерние процессы — ffmpeg недоступен.
    // Треки будут отдаваться в исходном формате без перекодирования.
    None
}
